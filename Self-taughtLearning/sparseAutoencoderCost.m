...
costSquaredError=mean(1/2*sum((a3-data).^2));
costWeightDecay=(sum(sum(W1.^2))+sum(sum(W2.^2)));

p=mean(a2,2);
costSparsityPenalty=sum(sparsityParam.*log(sparsityParam./p)+(1-sparsityParam).*log((1-sparsityParam)./(1-p)));
cost=costSquaredError+lambda/2*costWeightDecay+beta*costSparsityPenalty;
